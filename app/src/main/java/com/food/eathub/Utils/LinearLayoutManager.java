			...
			if (itemCount > 0) {
				...
				while (i < itemCount && acumulatedSize < maxSize) {
					...
					if (viewForPosition != null) {
						...
						acumulatedSize += (orientation == VERTICAL ? childHeight
								: childWidth);
					}
					i++;
				}
				acumulatedSize = Math.min(acumulatedSize, maxSize);
				int width = orientation == VERTICAL ? widthSize
						: acumulatedSize;
				int height = orientation == HORIZONTAL ? heightSize
						: acumulatedSize;
				measuredWidth = width;
