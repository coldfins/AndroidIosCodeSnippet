...
public class GradientOverImageDrawable extends BitmapDrawable {
	...
	private double gradientStart = 0.25;
	...
	public void setGradientColors(int startColor, int... endColors) {
		if (endColors.length == 0) {
			throw new IllegalArgumentException(
					"The endColors array must have at least one element");
		}
	...
	public void setGradientPositions(float[] gradientPositions) {
		for (float pos : gradientPositions) {
			if (pos > 1 || pos < 0) {
				throw new IllegalArgumentException(
						"The gradient position must be a float number between 0 and 1");
			}
		}
	...
	public void draw(Canvas canvas) {
		...
		if (gradientColors != null) {
			...
			LinearGradient shader = new LinearGradient(x0, y0, x1, y1,
					gradientColors, gradientPositions, Shader.TileMode.MIRROR);
