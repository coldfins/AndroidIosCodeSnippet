...
public class PLView extends Activity implements PLIView, SensorEventListener,
		OnDoubleTapListener {
	/** member variables */
	...
	private boolean mHasFirstGyroscopePitch, mHasFirstAccelerometerPitch,
			mHasFirstMagneticHeading;
	private float mFirstAccelerometerPitch, mLastAccelerometerPitch,
			mAccelerometerPitch;
	private float mFirstMagneticHeading, mLastMagneticHeading,
			mMagneticHeading;
	...
	public boolean reset(boolean resetCamera) {
		if (!mIsValidForTransition) {
			...
			mAuxiliarStartPoint.setValues(mAuxiliarEndPoint.setValues(0.0f,
					0.0f));
			mFovCounter = 0;
			mFovDistance = 0.0f;
		...
		if (!mIsValidForTransition) {
			...
			if (panorama != null) {
				...
				if (mRenderer != null) {
					...
					mGLSurfaceView = new PLSurfaceView(
							this.getApplicationContext(), mRenderer);
					mPanorama = panorama;
	...
	public CGRect getRenderingViewport() {
		return (mRenderer != null ? mTempRenderingViewport.setValues(mRenderer
				.getViewport()) : mTempRenderingViewport.reset());
	}

	@Override
	public CGSize getRenderingSize() {
		return (mRenderer != null ? mTempRenderingSize.setValues(mRenderer
				.getSize()) : mTempRenderingSize.reset());
	}
	...
	public void setAnimationFrameInterval(int animationFrameInterval) {
		if (animationFrameInterval >= 1) {
			...
			mAnimationInterval = PLConstants.kDefaultAnimationTimerIntervalByFrame
	...
	public void setAccelerometerSensitivity(float accelerometerSensitivity) {
		mAccelerometerSensitivity = PLMath.valueInRange(
				accelerometerSensitivity, PLRange.PLRangeMake(
						PLConstants.kAccelerometerSensitivityMinValue,
	...
	public void setNumberOfTouchesForReset(int numberOfTouchesForReset) {
		if (numberOfTouchesForReset > 2
				&& numberOfTouchesForReset <= kMaxTouches)
	...
	public boolean startAnimation() {
		if (!mIsAnimating) {
			...
			this.setAnimationTimer(NSTimer.scheduledTimerWithTimeInterval(
					mAnimationInterval, new NSTimer.Runnable() {
						@Override
		...
		if (touches.size() == 2) {
			...
			if (mFovCounter < PLConstants.kDefaultFovMinCounter) {
				if (mFovCounter == PLConstants.kDefaultFovMinCounter - 1)
					mFovDistance = PLMath.distanceBetweenPoints(
							mAuxiliarStartPoint, mAuxiliarEndPoint);
				return false;
			}

			float distance = PLMath.distanceBetweenPoints(mAuxiliarStartPoint,
					mAuxiliarEndPoint), distanceDiff = distance - mFovDistance;

			if (Math.abs(distanceDiff) < mPanorama.getCamera()
			...
			if (mListener != null)
				isNotCancelable = mListener.onShouldRunZooming(this,
						distanceDiff, isZoomIn, !isZoomIn);

			if (isNotCancelable) {
				mFovDistance = distance;
				mPanorama.getCamera().addFov(this, distanceDiff);
				if (mListener != null)
					mListener.onDidRunZooming(this, distanceDiff, isZoomIn,
							!isZoomIn);
				return true;
			}
		}
		...
		if (touchCount == mNumberOfTouchesForReset) {
			...
			if (isNotCancelable) {
				...
				if (eventType == PLTouchEventType.PLTouchEventTypeMoved)
					...
					mAuxiliarStartPoint.setValues(touches.get(0)
							.locationInView(mGLSurfaceView));
					mAuxiliarEndPoint.setValues(touches.get(1).locationInView(
							mGLSurfaceView));
					if (mListener != null)
						mListener.onDidBeginZooming(this, mAuxiliarStartPoint,
								mAuxiliarEndPoint);
				}
			}
			...
			if (eventType == PLTouchEventType.PLTouchEventTypeMoved) {
				if (mIsValidForFov
						|| (mStartPoint.x == 0.0f && mEndPoint.y == 0.0f))
					mStartPoint
							.setValues(this.getLocationOfFirstTouch(touches));
	...
	protected void touchesBegan(List<UITouch> touches, MotionEvent event) {
		...
		if (this.isLocked()
				|| mIsValidForCameraAnimation
				|| mIsValidForTransition
				|| !this.isTouchInView(touches)
				|| (listenerExists && !mListener.onShouldBeginTouching(this,
						touches, event)))
			return;
		...
		case 1:
			...
			if (mIsValidForScrolling) {
				if (mInertiaTimer != null)
					...
					if (listenerExists)
						mListener.onDidEndScrolling(this, mStartPoint,
								mEndPoint);
				}
			}
			break;
		...
		if (!this.executeDefaultAction(touches,
			...
			mEndPoint.setValues(mStartPoint.setValues(this
	...
	protected void touchesMoved(List<UITouch> touches, MotionEvent event) {
		...
		if (this.isLocked()
				...
				|| mIsValidForTransition
				|| !this.isTouchInView(touches)
				|| (listenerExists && !mListener.onShouldMoveTouching(this,
						touches, event)))
			return;
	...
	protected void touchesEnded(List<UITouch> touches, MotionEvent event) {
		...
		if (this.isLocked()
				...
				|| mIsValidForTransition
				|| !this.isTouchInView(touches)
				|| (listenerExists && !mListener.onShouldEndTouching(this,
						touches, event))) {
		...
		if (mIsValidForFov) {
			...
			if (!this.executeDefaultAction(touches,
				...
				if (mIsScrollingEnabled && listenerExists)
					isNotCancelable = mListener.onShouldBeingScrolling(this,
							mStartPoint, mEndPoint);

				if (mIsScrollingEnabled && isNotCancelable) {
					boolean isValidForMoving = (PLMath.distanceBetweenPoints(
							mStartPoint, mEndPoint) >= mMinDistanceToEnableScrolling);
					if (mIsInertiaEnabled) {
						if (isValidForMoving) {
							mIsValidForScrolling = true;
							isNotCancelable = true;
							if (listenerExists) {
								mListener.onDidBeginScrolling(this,
										mStartPoint, mEndPoint);
								isNotCancelable = mListener
										.onShouldBeginInertia(this,
												mStartPoint, mEndPoint);
							}
							if (isNotCancelable) {
	...
	protected void startInertia() {
		if (this.isLocked()
				|| mIsValidForInertia
				|| mIsValidForTransition
				|| (mListener != null && !mListener.onShouldRunInertia(this,
						mStartPoint, mEndPoint)))
			return;

		mIsValidForInertia = true;
		float interval = mInertiaInterval
				/ PLMath.distanceBetweenPoints(mStartPoint, mEndPoint);
		if (interval < 0.01f) {
			mInertiaStepValue = 0.01f / interval;
			interval = 0.01f;
		} else
			mInertiaStepValue = 1.0f;
		mInertiaTimer = NSTimer.scheduledTimerWithTimeInterval(interval,
	...
	protected void inertia() {
		if (this.isLocked() || mIsValidForCameraAnimation
				|| mIsValidForTransition)
			return;

		float m = (mEndPoint.y - mStartPoint.y) / (mEndPoint.x - mStartPoint.x);
		float b = (mStartPoint.y * mEndPoint.x - mEndPoint.y * mStartPoint.x)
				/ (mEndPoint.x - mStartPoint.x);
		float x, y, add;

		if (Math.abs(mEndPoint.x - mStartPoint.x) >= Math.abs(mEndPoint.y
				- mStartPoint.y)) {
			add = (mEndPoint.x > mStartPoint.x ? -mInertiaStepValue
					: mInertiaStepValue);
			x = mEndPoint.x + add;
			if ((add > 0.0f && x > mStartPoint.x)
					|| (add <= 0.0f && x < mStartPoint.x)) {
				this.stopInertia();
				return;
			}
			y = m * x + b;
		} else {
			add = (mEndPoint.y > mStartPoint.y ? -mInertiaStepValue
					: mInertiaStepValue);
			y = mEndPoint.y + add;
			if ((add > 0.0f && y > mStartPoint.y)
					|| (add <= 0.0f && y < mStartPoint.y)) {
				this.stopInertia();
				return;
			}
			x = (y - b) / m;
	...
	protected boolean activateAccelerometer() {
		if (mSensorManager != null
			...
			return true;
		PLLog.debug("PLView::activateAccelerometer",
	...
	protected void accelerometer(SensorEvent event, UIAcceleration acceleration) {
		if (this.isLocked() || this.resetWithShake(acceleration)
				|| mIsValidForTouch || mIsValidForScrolling
				|| mIsValidForSensorialRotation || mIsValidForCameraAnimation
				|| mIsValidForTransition)
			return;

		if (mIsAccelerometerEnabled) {
			if (mListener != null
					&& !mListener.onShouldAccelerate(this, acceleration, event))
				return;

			float x = 0.0f, y = (mIsAccelerometerUpDownEnabled ? -acceleration.z
					: 0.0f), factor = mAccelerometerSensitivity
					* (mPanorama.getCamera().isReverseRotation() ? -PLConstants.kAccelerometerMultiplyFactor
			...
			mPanorama.getCamera().rotate(this, mAuxiliarStartPoint,
					mAuxiliarEndPoint);
	...
	protected void deactivateMagnetometer() {
		if (mSensorManager != null)
			mSensorManager
					.unregisterListener(this, mSensorManager
	...
	public boolean startSensorialRotation() {
		if (!mIsValidForSensorialRotation) {
			if (this.activateGyroscope()) {
				...
				PLLog.debug("PLView::startSensorialRotation",
						"Gyroscope sensor is not available on device!");
				if (mSensorManager != null
					...
					PLLog.debug("PLView::startSensorialRotation",
							"Accelerometer or/and magnetometer sensor/s is/are not available on device!");
			}
			return mIsValidForSensorialRotation;
		}
		return false;
	...
	protected void doGyroUpdate(float pitch, float yaw) {
		if (this.isLocked() || mIsValidForTouch || mIsValidForScrolling
				|| mIsValidForCameraAnimation || mIsValidForTransition
				|| !mHasFirstGyroscopePitch)
			return;
	...
	protected void doSimulatedGyroUpdate() {
		...
		float step, offset = Math.abs(mLastAccelerometerPitch
				- mAccelerometerPitch);
		if (offset < 0.25f)
		...
		mPanorama.getCamera().lookAt(this, mAccelerometerPitch,
				mMagneticHeading);
	}
	...
	protected void updateGyroscopeRotationByOrientation(
	...
	protected boolean activateOrientation() {
		if (mSensorManager != null
			...
			return true;
		PLLog.debug("PLView::activateOrientation",
				"Orientation sensor is not available on the device!");
		return false;
	}
	...
	protected boolean resetWithShake(UIAcceleration acceleration) {
		if (!mIsShakeResetEnabled || !mIsResetEnabled || this.isLocked()
				|| mIsValidForCameraAnimation || mIsValidForTransition)
			return false;
		...
		if ((currentTime - mShakeData.lastTime) > PLConstants.kShakeDiffTime) {
			...
			float speed = Math.abs(mShakeData.shakePosition.x
					+ mShakeData.shakePosition.y + mShakeData.shakePosition.z
					- mShakeData.shakeLastPosition.x
					- mShakeData.shakeLastPosition.y
					- mShakeData.shakeLastPosition.z)
					/ diffTime * 10000;
	...
	@Override
	public boolean startTransition(PLITransition transition,
			PLIPanorama newPanorama) {
		if (mIsValidForTransition || mPanorama == null || mRenderer == null
				|| transition == null)
			return false;
		...
		mCurrentTransition = transition;
		mCurrentTransition.setInternalListener(new PLTransitionListener() {
			@Override
			...
			public void didEndTransition(PLITransition transition) {
				...
				if (mListener != null)
					mListener.onDidEndTransition(transition.getView(),
							transition);
			}
		});
	...
	public void load(PLILoader loader, boolean showProgressBar) {
		this.load(loader, showProgressBar, null,
				PLConstants.kFloatUndefinedValue,
				PLConstants.kFloatUndefinedValue);
	}
	...
	public void load(PLILoader loader, boolean showProgressBar,
		...
		this.load(loader, showProgressBar, transition,
				PLConstants.kFloatUndefinedValue,
				PLConstants.kFloatUndefinedValue);
	}
	...
	public void load(final PLILoader loader, boolean showProgressBar,
			final PLITransition transition, final float initialPitch,
			final float initialYaw) {
		if (loader != null) {
			mFileDownloaderManager.removeAll();
			loader.setInternalListener(new PLLoaderListener() {
				@Override
	...
	protected class PLInternalCameraListener implements PLCameraListener,
			PLIReleaseView {
		/** member variables */
		...
		@Override
		public void didLookAt(Object sender, PLICamera camera, float pitch,
				float yaw, boolean animated) {
			if (sender != mView)
				mView.updateInitialSensorialRotation();
			PLViewListener listener = mView.getListener();
			if (listener != null)
				listener.onDidLookAtCamera(mView, sender, camera, pitch, yaw,
						animated);
		}

		@Override
		public void didRotate(Object sender, PLICamera camera, float pitch,
				float yaw, float roll) {
			if (sender != mView)
				mView.updateInitialSensorialRotation();
			PLViewListener listener = mView.getListener();
			if (listener != null)
				listener.onDidRotateCamera(mView, sender, camera, pitch, yaw,
						roll);
		}
	...
	private static final int kMaxTouches = 20;
	...
	public CGSize getSize() {
		...
		return mTempSize.setValues(displayMetrics.widthPixels,
	 ...
	 * 
	 * @param glSurfaceView
	 *            current GLSurfaceView
	 */
	@SuppressWarnings("deprecation")
	protected View onGLSurfaceViewCreated(GLSurfaceView glSurfaceView) {
		for (int i = 0; i < kMaxTouches; i++)
			mInternalTouches.add(new UITouch(glSurfaceView, new CGPoint(0.0f,
					0.0f)));
		mContentLayout = new RelativeLayout(this);
		mContentLayout.setLayoutParams(new LayoutParams(
				LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT));
		mContentLayout.addView(glSurfaceView, new LayoutParams(
				LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT));
	...
	protected void onResume() {
		...
		if (mIsValidForSensorialRotation) {
			...
			if (mSensorialRotationType == PLSensorialRotationType.PLSensorialRotationTypeGyroscope)
				...
				mSensorialRotationThresholdTimestamp = System
	...
	public void onSensorChanged(SensorEvent event) {
		...
		case Sensor.TYPE_ACCELEROMETER:
			if (mIsRendererCreated && mRenderer.isRunning()
					&& !mIsValidForTransition) {
		...
		case Sensor.TYPE_ORIENTATION:
			...
			case Configuration.ORIENTATION_PORTRAIT:
				switch (this.getWindowManager().getDefaultDisplay()
						.getOrientation()) {
			...
			if (mCurrentDeviceOrientation != newOrientation) {
				if (mIsValidForSensorialRotation
						&& mSensorialRotationType == PLSensorialRotationType.PLSensorialRotationTypeGyroscope)
					this.updateGyroscopeRotationByOrientation(
		...
		case Sensor.TYPE_MAGNETIC_FIELD:
			if (mIsRendererCreated && mRenderer.isRunning()
				...
				if (mIsValidForSensorialRotation
						&& mSensorialRotationType == PLSensorialRotationType.PLSensorialRotationTypeAccelerometerAndMagnetometer
						&& mSensorialRotationAccelerometerData != null) {
					if (mSensorialRotationThresholdFlag) {
						SensorManager.getRotationMatrix(
								mSensorialRotationRotationMatrix, null,
								mSensorialRotationAccelerometerData, values);
						...
						float yaw = mSensorialRotationOrientationData[0]
								* PLConstants.kToDegrees;
						float pitch = -mSensorialRotationOrientationData[1]
								* PLConstants.kToDegrees;
						if (mHasFirstMagneticHeading) {
							if ((pitch >= 0.0f && pitch < 50.0f)
									|| (pitch < 0.0f && pitch > -50.0f)) {
								yaw -= mFirstMagneticHeading;
								float diff = yaw - mLastMagneticHeading;
								if (Math.abs(diff) > 100.0f) {
									mLastMagneticHeading = yaw;
									mMagneticHeading += (diff >= 0.0f ? 360.0f
											: -360.0f);
								} else if ((yaw > mLastMagneticHeading && yaw
										- PLConstants.kSensorialRotationYawErrorMargin > mLastMagneticHeading)
										|| (yaw < mLastMagneticHeading && yaw
												+ PLConstants.kSensorialRotationYawErrorMargin < mLastMagneticHeading))
									mLastMagneticHeading = yaw;
							}
						} else {
							float cameraYaw = mPanorama.getCamera()
									.getLookAtRotation().yaw;
						...
						if (mHasFirstAccelerometerPitch) {
							...
							if ((pitch > mLastAccelerometerPitch && pitch
									- PLConstants.kSensorialRotationPitchErrorMargin > mLastAccelerometerPitch)
									|| (pitch < mLastAccelerometerPitch && pitch
											+ PLConstants.kSensorialRotationPitchErrorMargin < mLastAccelerometerPitch))
								mLastAccelerometerPitch = pitch;
						} else {
							float cameraPitch = mPanorama.getCamera()
									.getLookAtRotation().pitch;
			...
			if (mIsRendererCreated && mRenderer.isRunning()
				...
				if (mHasFirstGyroscopePitch) {
					if (mGyroscopeLastTime != 0) {
						float timeDiff = (event.timestamp - mGyroscopeLastTime)
						...
						case UIDeviceOrientationUnknown:
						case UIDeviceOrientationPortrait:
							this.doGyroUpdate(mGyroscopeRotationX
									* PLConstants.kToDegrees,
									-mGyroscopeRotationY
											* PLConstants.kToDegrees);
							break;
						case UIDeviceOrientationLandscapeLeft:
							this.doGyroUpdate(-mGyroscopeRotationY
									* PLConstants.kToDegrees,
									-mGyroscopeRotationX
											* PLConstants.kToDegrees);
							break;
					...
					PLRotation cameraRotation = mPanorama.getCamera()
							.getLookAtRotation();
					switch (mCurrentDeviceOrientation) {
					case UIDeviceOrientationUnknown:
					case UIDeviceOrientationPortrait:
						mGyroscopeRotationX = cameraRotation.pitch
								* PLConstants.kToRadians;
						mGyroscopeRotationY = -cameraRotation.yaw
								* PLConstants.kToRadians;
						break;
					case UIDeviceOrientationLandscapeLeft:
						mGyroscopeRotationX = -cameraRotation.yaw
								* PLConstants.kToRadians;
						mGyroscopeRotationY = -cameraRotation.pitch
								* PLConstants.kToRadians;
						break;
					case UIDeviceOrientationLandscapeRight:
						mGyroscopeRotationX = cameraRotation.yaw
